...
for ik in range(3):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        network = Net(input_s, hidden_1_s, hidden_2_s[ik], output_s).to(device)
...
fig = plt.figure()
ax0 = fig.add_subplot()
ax0.set_title("CE Loss for different 2nd hidden layer architectures")
ax0.xaxis.set_major_locator(MaxNLocator(integer=True))
plt.xlabel('Epoch', fontsize=8)
plt.ylabel('Loss', fontsize=8)
decorate_axis(ax0)
h_1 = plt.plot(x,all_losses[0],label = "H2 = H1")
h_2 = plt.plot(x,all_losses[1],label = "H2 = H1/2")
h_3 = plt.plot(x,all_losses[2], label = "H2 = 2H1")
lgnd = ax0.legend(handles=[h_1[0], h_2[0], h_3[0]],loc = "upper right")
fig.savefig('ce_2.png')
