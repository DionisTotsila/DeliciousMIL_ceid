...
#ANN
class Net(nn.Module):
  def __init__(self,input_size, H_1, classes):
    super(Net, self).__init__()
    self.fc1 = nn.Linear(input_size,  H_1)
    self.fc2 = nn.Linear(H_1, classes)    
...
output_s = 20
input_s = 8520
hidden_1_s = [ output_s, (output_s + input_s)//2, output_s + input_s ]
# hidden_2_s = [ hidden_1_s, (hidden_1_s)//2,  2*hidden_1_s]
...
data_X = np.vstack((data_train['X'], data_test['X']))
...
for ik in range(3):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        network = Net(input_s, hidden_1_s[ik], output_s).to(device)
    ...
    for key, value in results.items():
        ...
        temp_sum += value
    print(f'Average: {temp_sum/len(results.items())} %')

    all_losses.append(np.mean(np.array(losses).reshape(k_folds,num_epochs), axis = 0))
...
fig = plt.figure()
ax0 = fig.add_subplot()
ax0.set_title("MSE Loss for different 1st hidden layer architectures")
ax0.xaxis.set_major_locator(MaxNLocator(integer=True))
plt.xlabel('Epoch', fontsize=8)
plt.ylabel('Loss', fontsize=8)
decorate_axis(ax0)
h_1 = plt.plot(x,all_losses[0],label = "H1 = O")
h_2 = plt.plot(x,all_losses[1],label = "H1 = (I+O)/2")
h_3 = plt.plot(x,all_losses[2], label = "H1 = I+O")
lgnd = ax0.legend(handles=[h_1[0], h_2[0], h_3[0]],loc = "upper right")
fig.savefig('mse_1.png')
