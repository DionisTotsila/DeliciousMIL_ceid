...
def accuracy(output_1, target_1):
    ...
    accur =  sum_1 / sum_2
    #use this to handle 0/0 no need for try except
    if(sum_2 == sum_1):
        accur = 1
...
for ik in range(3):
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        optimizer = torch.optim.SGD(network.parameters(), lr=lr, momentum = m, weight_decay=w[ik])
...
fig = plt.figure()
ax0 = fig.add_subplot()
ax0.set_title("MSE Loss for different lr and momentum")
ax0.xaxis.set_major_locator(MaxNLocator(integer=True))
plt.xlabel('Epoch', fontsize=8)
plt.ylabel('Loss', fontsize=8)
decorate_axis(ax0)
h_1 = plt.plot(x,all_losses[0], label = "wd = 0.1")
h_2 = plt.plot(x,all_losses[1], label = "wd = 0.5")
h_3 = plt.plot(x,all_losses[2], label = "wd = 0.9")
lgnd = ax0.legend(handles=[h_1[0], h_2[0], h_3[0]],loc = "upper right")
fig.savefig('mse_wd.png')
